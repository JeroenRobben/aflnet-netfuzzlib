...
# ASan under AFL: abort (so AFL sees a crash), and disable the link-order
# check since the non-ASan libnfl-aflnet.so is AFL_PRELOADed ahead of libasan.
export ASAN_OPTIONS="abort_on_error=1:verify_asan_link_order=0:detect_leaks=0:symbolize=0:handle_segv=0"
